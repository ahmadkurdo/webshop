...
export const setUpdater = makeFun<React.Dispatch<React.SetStateAction<AppState>>, SetStateAction<AppState>>(
    (setState: React.Dispatch<React.SetStateAction<AppState>>) => (state: AppState) =>
        Entity(state)
            .set('updateAppState', (uap) => setState)
            .commit()
)
export const updateSearchItems = makeFun<Action<Product[]>, SetStateAction<AppState>>(
...
export const filterProduct = makeFun<string, Action<Product[]>>((querystring: string) => (products: Product[]) =>

...
export const makeFun = <input, output>(
  f: (_: input) => output
): Fun<input, output> => {
...
export const setUpdater = makeFun<
  React.Dispatch<React.SetStateAction<AppState>>,
  SetStateAction<AppState>
>(
...
export const updateSearchItems = makeFun<
  Action<Product[]>,
  SetStateAction<AppState>
...
export const filterProduct = makeFun<string, Action<Product[]>>(
  (querystring: string) => (products: Product[]) =>
    products.filter(
      (product) => product.name !== "" && product.name.includes(querystring)
    )
);

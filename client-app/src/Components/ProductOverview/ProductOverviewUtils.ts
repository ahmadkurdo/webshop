...
export const setSearchBarItems = makeFun<Product[],SetStateAction<AppState>>((newValue : Product[]) => (state : AppState) => Entity(state).setIn("productOverviewState" , povs => povs.setIn("searchbarState", sbs => sbs.set("items", _ => newValue))).commit())

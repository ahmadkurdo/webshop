...
export const setProducts = makeFun<Product[], SetStateAction<AppState>>(
...
export const setProductOverView = makeFun<
  ...
  (newValue: Product[]) => (state: AppState) =>
    Entity(state)
      .setIn("headerState", (povs) =>
        povs.setIn("searchbarState", (sbs) => sbs.set("items", (_) => newValue))
      )
      .commit()
);
